            ...
            targets: [
              {
                ...
                expr: 'label_replace(memberlist_client_cluster_node_health_score{namespace="$namespace"}, "pod", "$1", "pod", "observatorium.+(compactor.*|distributor.*|ingester.*|querier.*).*")',
                instant: false,
                interval: '',
                legendFormat: '{{ pod }}',
                refId: 'A',
            ...
            targets: [
              {
                ...
                expr: '(\n  label_replace(rate(memberlist_client_cas_success_total{namespace="$namespace"}[5m]), "pod", "$1", "pod", "observatorium.+(compactor.*|distributor.*|ingester.*|querier.*).*")\n/\n  label_replace(rate(memberlist_client_cas_attempt_total{namespace="$namespace"}[5m]), "pod", "$1", "pod", "observatorium.+(compactor.*|distributor.*|ingester.*|querier.*).*")\n) \n> -1\n',
                interval: '',
                legendFormat: '{{ pod }}',
                refId: 'A',
              },
            ...
            targets: [
              {
                ...
                expr: 'count(count by(pod)({namespace="$namespace", pod=~".+loki.+(compactor|ingester|querier|distributor).*"}))',
                interval: '',
              ...
              },
              {
                ...
                expr: 'bottomk(1, memberlist_client_cluster_members_count{namespace="$namespace"})',
                interval: '',
          ...
          list: [
            {
              ...
              queryValue: '',
              refresh: 1,
              regex: '(app-sre-stage|telemeter-prod)-01-prometheus',
              skipUrlSync: false,
              type: 'datasource',
            },
            {
